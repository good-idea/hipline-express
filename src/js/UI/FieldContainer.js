...
const singleToArray = R.when(a => a.constructor !== Array, a => [a])

const HelpText = ({ text, classNames }) => (text ? <h5 className={cn('field__helptext', classNames)}>{text}</h5> : null)
...
class FieldContainer extends React.Component {
  renderHelpText() {
    ...
    return singleToArray(this.props.helptext).map(text => <HelpText key={text} text={text} />)
  }
      ...
      <div className={cn(this.props.classNames, 'field', additionalClassNames)}>
        ...
        <div className="field__content">
          <div className="field__inputWrapper">
            <Field {...this.props} initialValue={this.props.initialValue} />
            {this.renderHelpText()}
            {validationText}
          </div>
        </div>
