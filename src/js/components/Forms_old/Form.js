...
class Form extends React.Component {
  ...
  onSubmit(e) {
    e.preventDefault()
    this.props.handleSubmit(this.fields.reduce((acc, current) => R.assoc(current.name, current.state.value, acc), {}))
  }
  ...
  updateFieldState(name, newState) {
    ...
    this.fields = [...this.fields.slice(0, index), newField, ...this.fields.slice(index + 1)]
    const values = this.getFormValues()
    this.fields.map(f => {
      f.onFormUpdate(values)
    })

    // Check to see if the form is valid
    const valid = R.filter(R.pathEq(['state', 'valid'], false))(this.fields).length === 0
