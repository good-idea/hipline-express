...
class DanceClass extends React.Component {
  ...
  render() {
    ...
    const signUp = isFuture(this.props.startTime) ? <button>Sign Up</button> : null

    return (
      <div className={cn(classNames)}>
        <h2 className="schedule__class__title">{this.props.title}</h2>
        <div className="schedule__class__inner">
          ...
          <div className="schedule__class__signup">{signUp}</div>
        </div>
      </div>
    )
  }
