...
class Login extends React.Component {
  ...
  handleUserForgot = userInfo => {
    ...
    axios
      .post('/api/mbo/forgot', serializedUserInfo)
      .then(response => {
        this.setState({
          forgotMessage:
            'Check your email for instructions on resetting your password on the MindBodyOnline website.\nAfter creating a new password, you can return here to log in.',
  ...
  handleUserLogin = userInfo => {
    const serializedUserInfo = serializeForSoap(userInfo)
    axios
      .post('/api/mbo/login', serializedUserInfo)
      .then(response => {
  ...
  render() {
    ...
    const form = this.state.showForgot ? (
      ...
      <LoginForm message={this.state.loginMessage} onSubmit={this.handleUserLogin} fieldConfig={loginFields} />
    )
    ...
    return (
      <section className={cn(classNames)}>
        <div className="column">
          ...
          {form}
          <button className="secondary login__toggleForgot" onClick={this.toggleForgot}>
            {toggleText}
          </button>
        </div>
      </section>
